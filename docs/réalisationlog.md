...
Nous avons aussi pu validé la communication descendante avec l'outil de visualisation en direct  de TIA Portal. Les coordonnées calculées par Python sont bien transmises vers l'automate, qui les réceptionne et les stocke dans le bloc de données (DB) interne, ce qui doit permettre de les envoyer au robot par la suite.
